...
- When a side effect has a canonical post-commit owner, remove duplicate call sites rather than adding deduplication state to mask them. Check that each remaining caller is genuinely single-shot before accepting the simplification.
