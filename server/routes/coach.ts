// The squad-building coach. Port of `convex/http.ts`'s `/coach` action.
//
// Two differences from the Convex version, both consequences of the move:
//
//  1. NO CORS. On Convex this endpoint lived on a different origin from the app
//     and needed `Access-Control-Allow-*` plus a preflight OPTIONS route. It is
//     same-origin now (Vite proxies `/api` in dev, Elysia serves the bundle in
//     prod), so the headers and the OPTIONS handler are deleted rather than
//     carried over — a wildcard ACAO on a same-origin endpoint is pure downside.
//  2. It streams straight through: the handler returns the AI SDK's `Response`
//     and Elysia passes the body along untouched.
//
// The OpenRouter key travels in `x-openrouter-key` and is used to build the
// provider for this one request. It is never persisted, never logged, and never
// echoed back — that contract is the reason it is a header and not a body field.

import { Elysia } from "elysia";
import { convertToModelMessages, streamText, tool, type UIMessage } from "ai";
import { createOpenRouter } from "@openrouter/ai-sdk-provider";
import { z } from "zod";
import { BASE_STATS, CATALOG, GRID_SIZE, TURN_CAP } from "../../shared/catalog";
...
export const coachRoutes = new Elysia().post("/coach", async ({ request, status }) => {
  ...
  if (!apiKey || !model) {
    return status(400, { error: "missing x-openrouter-key or x-model header" });
  }

  const { messages } = (await request.json()) as { messages: Array<UIMessage> };
